...
- 共 19 頁；右側為放大的 SVG 面積圖或分類卡片，左側為較小標題與醒目的關鍵公式。
- 已發布到 GitHub Pages 的 repository 根目錄：`https://imrchen.github.io/rectangle-formula-slides/`。
...
以 Node 對內嵌 JavaScript 執行語法解析，並確認投影片物件數量為 19。人工於桌面瀏覽器檢查首尾頁、撕紙條動畫、差的平方頁、和平方／差平方比較頁、平方差頁、結尾的心算範例頁與公式判斷頁與鍵盤導覽；手機版採響應式單欄顯示。
